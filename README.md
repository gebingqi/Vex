...
- Save configuration: `vex save <name> [-y] [-d "desc"] <qemu-bin> [qemu args ...]` â€”â€?Save QEMU startup parameters as configurations, eliminating repetitive input. Can overwrite existing names with prompts, -y forces overwrite, and you can add a description for the configuration (optional) using double quotes;
- Rename configuration: `vex rename [-y] [-d "desc"] <old_name> <new_name>` â€”â€?Rename saved configurations, maintaining name uniqueness and readability;
- Execute configuration: `vex exec <name>` â€”â€?Directly execute saved configurations, one-click QEMU startup;
- View configurations: `vex list` â€”â€?List all saved configurations;
- Delete configuration: `vex rm <name>` â€”â€?Remove unused configurations, keeping the local environment clean.
- Edit configuration: `vex edit <name>` â€”â€?Modify the configuration interactively, with an option to test-run (trial execution) after editing. 
...
- Pull configuration: `vex pull <id/name>:[tag]` â€”â€?Pull configurations shared by others from remote sources, quickly reusing mature environments;
- Push configuration: `vex push <id/remote_name>:[tag] <local_name>` â€”â€?Push local configurations to remote sources, facilitating team sharing or cross-device usage.
